x# Module 4 Homework: Analytics Engineering with dbt
...
```sql
SELECT count(1) FROM `taxi-rides-ny-454801.zoomcamp.fct_monthly_zone_revenue`;
```

Note: I get the wrong result 11,814 probably because I messed up with data loading for BigQuery. I used module 2 where we load data via Kestra but forgot to exclude 2021 at the first place.
...
FROM `taxi-rides-ny-454801.zoomcamp.fct_monthly_zone_revenue`
WHERE 
  service_type = 'Green'
...
SELECT DISTINCT
  pickup_zone,
  SUM(revenue_monthly_total_amount) OVER (PARTITION BY pickup_zone) AS total_zone_revenue
FROM `taxi-rides-ny-454801.zoomcamp.fct_monthly_zone_revenue`
WHERE
  service_type = 'Green'
  AND revenue_month >= '2020-01-01'
